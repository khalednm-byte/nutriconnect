...
import { recipes as initialRecipes, dietFilters } from '../data/meals';
import {
  FiStar, FiClock, FiUsers, FiX, FiEdit2, FiYoutube,
  FiImage, FiChevronLeft, FiAlertCircle, FiPlus, FiTrash2,
} from 'react-icons/fi';
import './Recipes.css';

// ─── Tiny Markdown renderer ───────────────────────────────────────────────────
// Handles: ## headings, **bold**, - bullet lists, ![alt](url) images, newlines.
// No library needed — just string splitting and mapping to JSX.
function renderMarkdown(text) {
  if (!text) return null;
  const lines = text.split('\n');
  const elements = [];
  let listBuffer = []; // accumulates consecutive bullet lines

  const flushList = (key) => {
    if (listBuffer.length === 0) return;
    elements.push(
      <ul key={`ul-${key}`} className="recipe-instruction-list">
  ...
  lines.forEach((line, i) => {
    ...
    if (line.startsWith('## ')) {
      flushList(i);
      elements.push(<h4 key={i} className="recipe-md-heading">{line.slice(3)}</h4>);
    }
    // Bullet point
    ...
    // Image: ![alt](url)
      ...
      elements.push(
        <img key={i} src={url} alt={alt} className="recipe-md-image"
    ...
    // Regular paragraph
      ...
      flushList(i);
      elements.push(<p key={i} className="recipe-md-p">{inlineFormat(line)}</p>);
    }
  });
...
// Handles **bold** inline formatting within a line
...
// ─── Recipe emoji helper (same logic as before) ───────────────────────────────
...
// ─── Edit modal default form state built from a recipe object ─────────────────
function recipeToForm(recipe) {
  return {
    ...
    instructions: recipe.instructions
      // If the recipe has old-style array instructions, convert to markdown bullets
...
export default function Recipes() {
  ...
  const [editing, setEditing]           = useState(false); // edit modal open
  const [form, setForm]                 = useState({});    // edit form state
  ...
  }), [recipeList, activeFilter, search]);
  ...
  const handleSave = () => {
    ...
    if (Number(form.calories) < 0) return setFormError('Calories cannot be negative.');
    ...
    const updated = {
      ...
      rating:      Math.min(5, Math.max(0, Number(form.rating))) || selected.rating,
      dietType:    form.dietType.split(',').map(s => s.trim().replace(/ /g, '_')).filter(Boolean),
      ingredients: form.ingredients.split('\n').map(s => s.trim()).filter(Boolean),
      // Instructions stay as markdown string — the renderer handles it
      instructions: form.instructions,
      videoUrl:    form.videoUrl.trim(),
    };

    setRecipeList(prev => prev.map(r => r.id === selected.id ? updated : r));
    setSelected(updated); // update the detail view immediately
    setEditing(false);
    setFormError('');
  };
    ...
    <div className="recipes-page">
      ...
      <div className={`recipes-main ${selected ? 'detail-open' : ''}`}>
        ...
        <div className="recipes-grid">
          {filtered.length === 0 && (
            <p style={{ color: 'var(--text-muted)', gridColumn: '1/-1' }}>No recipes match your search.</p>
          )}
          {filtered.map(recipe => (
            <div
              key={recipe.id}
              className={`recipe-card card card-interactive ${selected?.id === recipe.id ? 'recipe-card-active' : ''}`}
            ...
            <div className="detail-body">
              ...
              <div className="detail-stats">
                <div className="detail-stat">
                  <FiClock />
                  <div>
                    <strong>{selected.prepTime + selected.cookTime} min</strong>
                ...
                <div className="detail-stat">
                  <FiUsers />
                  <div>
                    <strong>{selected.servings}</strong>
                    <span>Servings</span>
                  </div>
                </div>
                <div className="detail-stat">
              ...
              <div className="detail-macros">
                {[
                  { label: 'Calories', value: selected.calories, unit: '' },
                  { label: 'Protein',  value: selected.macros.protein, unit: 'g' },
                  { label: 'Carbs',    value: selected.macros.carbs,   unit: 'g' },
                  { label: 'Fat',      value: selected.macros.fat,     unit: 'g' },
                ].map(m => (
              ...
              {selected.videoUrl && !getYouTubeEmbed(selected.videoUrl) && (
                <div className="detail-section">
                  <h4><FiYoutube /> Recipe Video</h4>
                  <a href={selected.videoUrl} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-sm">
                    Watch Video ↗
                  </a>
                </div>
              )}
                ...
                <div className="detail-section">
                  ...
                  <div className="detail-instructions">
                    {Array.isArray(selected.instructions)
                      // Legacy array format from the original data
                      ? <ol className="recipe-instruction-list recipe-instruction-ol">
                          {selected.instructions.map((step, i) => <li key={i}>{step}</li>)}
                        </ol>
                      // Markdown string from admin edits
        ...
        <div className="modal-overlay" onClick={() => setEditing(false)}>
          <div className="modal edit-recipe-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <div>
                <h3>Edit Recipe</h3>
                <p className="modal-subtitle">{selected?.name}</p>
              </div>
            ...
            <div className="edit-modal-body">
              ...
              <section className="edit-section">
                ...
                <div className="edit-field-grid">
                  <label className="edit-field edit-field-full">
                    ...
                    <input value={form.name} onChange={e => handleField('name', e.target.value)} placeholder="Recipe name" />
                  </label>
                  <label className="edit-field edit-field-full">
                    <span>Author Name *</span>
                    <input value={form.authorName} onChange={e => handleField('authorName', e.target.value)} placeholder="Author" />
                  </label>
                  <label className="edit-field">
                    <span>Difficulty</span>
                    <select value={form.difficulty} onChange={e => handleField('difficulty', e.target.value)}>
                  ...
                  <label className="edit-field">
                    ...
                    <input type="number" min="0" max="5" step="0.1"
                      value={form.rating} onChange={e => handleField('rating', e.target.value)} />
                  </label>
                  <label className="edit-field">
                    <span>Prep Time (min)</span>
                    <input type="number" min="0" value={form.prepTime} onChange={e => handleField('prepTime', e.target.value)} />
                  </label>
                  <label className="edit-field">
                    <span>Cook Time (min)</span>
                    <input type="number" min="0" value={form.cookTime} onChange={e => handleField('cookTime', e.target.value)} />
                  </label>
                  <label className="edit-field">
                    <span>Servings</span>
                    <input type="number" min="1" value={form.servings} onChange={e => handleField('servings', e.target.value)} />
                  </label>
                  <label className="edit-field edit-field-full">
                    <span>Diet Types (comma-separated)</span>
                    <input value={form.dietType} onChange={e => handleField('dietType', e.target.value)}
                      placeholder="e.g. vegan, high_protein, gluten_free" />
                  </label>
                </div>
              </section>
              ...
              <section className="edit-section">
                ...
                <textarea
                  ...
                  placeholder={"## Preparation\n- Season the chicken with salt and pepper\n- **Tip:** Let it rest at room temperature for 10 min\n\n## Cooking\n- Grill for 6-7 minutes per side"}
                />
              </section>
              ...
              <section className="edit-section">
                <h4 className="edit-section-title">
                  <FiYoutube /> Video Link <span className="edit-optional">(Optional)</span>
                </h4>
                <p className="edit-hint">Paste a YouTube URL. It will be embedded in the recipe.</p>
                <input
                  className="edit-url-input"
                  type="url"
                  value={form.videoUrl}
                  onChange={e => handleField('videoUrl', e.target.value)}
